#!/bin/bash
# Test Suite for Snippet: websearch
# Generated: $(date)
# Pattern: \b(websearch|web[\s-]?search|internet[\s-]?search|online[\s-]?search|google|search[\s-]?web|query[\s-]?optimization)\b
# Verification Hash: 3bae729dadda6368

set -e

SNIPPET_NAME="websearch"
TEST_KEYWORD="websearch"
...
if [ -z "$snippet_path" ]; then
    snippet_path="snippets/websearch.md"
fi
